...
def main():
    ...
    processes = []
    numCores = multiprocessing.cpu_count()
    sharedList = ['a'] * numCores  
    for i in range (0,numCores):
        proc = multiprocessing.Process(target=procSync,args=(i,sharedList,),daemon=True)
        processes.append(proc)
        proc.start()
...
def procSync(index,sharedList):
    sharedList[index] = index 
    print("Process ",index," here: my list is ",sharedList)
